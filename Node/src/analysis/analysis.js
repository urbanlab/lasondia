...
var max_speed = 150;
var speed_step = 10;
var zoom_percentages = ['100%', '50%', '25%', '12.5%'];
var zoom_index = 0;
...
window.onload = function() {
    ...
    $('.control_button').each(function() {
        change_svg_color(this, 'white');
    });

    addEventListener_to_svg($('#loop')[0], 'click', function() {
        if(listen_mode == 'continuous') {
            change_svg_color($('#loop')[0], 'red');
            loop_mode();
        } else {
            change_svg_color($('#loop')[0], 'white');
            continuous_mode();
        }
    });

    addEventListener_to_svg($('#previous')[0], 'click', previous_landmark);

    addEventListener_to_svg($('#play')[0], 'click', function() {
        if(p.player._isPlaying) {
            p.player.pause();
            change_svg_color($('#play')[0], 'white');   
        } else {
            p.player.play();
            change_svg_color($('#play')[0], 'red');
        }
    });

    addEventListener_to_svg($('#next')[0], 'click', next_landmark);
...
//-------------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------------
// Utility functions for calculations and more
//-------------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------------
...
function change_svg_color(svg_object, color) {
    // Change the color of the SVG image contained in the 'object'
    // node svg_object
    var inner_svg = svg_object.getSVGDocument().activeElement;
    inner_svg.style.fill = color;
}

function addEventListener_to_svg(svg_object, event_type, callback) {
    // Add an event listener to the SVG image contained in the 'object'
    // node svg_object (adding it directly to svg_object won't work)
    var inner_svg = svg_object.getSVGDocument().activeElement;
    inner_svg.addEventListener(event_type, callback);
}

function change_volume() {
    // Function called when the volume slider is moved; adapt the audio volume.
    var volume = $('#sound_control');
...
function zoomOut() {
    // Function called when te 'Zoom -' button is clicked.
    if(zoom_index == zoom_percentages.length - 1) {
        return
    } else {
        p.zoom.zoomOut();
        zoom_index ++;
        $('#zoom_indication').text(zoom_percentages[zoom_index]);
    }
}
...
function increase_speed() {
    // Function called when te 'Speed +' button is clicked. Increase
    // the audio speed without affecting the pitch.
    if(audio_speed == max_speed) {
        return
    } else {
        audio_speed += speed_step;
        $('audio')[0].playbackRate = audio_speed / 100;
        $('#speed_indication').text(String(audio_speed) + '%');
    }
}

function decrease_speed() {
    // Function called when te 'Speed -' button is clicked. Decrease
    // the audio speed without affecting the pitch.
