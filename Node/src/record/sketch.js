...
// Accelerate p5.js by disabling the Friendly Erors
...
function draw() {
    ...
    duration = new Date(currentTime - startTime);
		...
		color = [100, 100, 200, 50]; // while recording, the timeline appears in blue
	} else {
		update_timeline_contents(false);
    color = [100, 100, 100, 50]; // while not recording, it appears in grey
	}
	for(var i = 0; i < timeline_size; i++) {
		...
		if(gommette_line[i] != null) {
			...
			image(gommette_icon, x, y_gommettes, 40, 64);
		}
	};
...
function update_timeline_contents(is_recording) {
	...
	var gommette;
	if(is_recording) {
		volume = liveVolume || 0; // liveVolume is defined in recorderWav.js, it is the current audio volume. This is the only variable from recorderWav.js that we use here (we use it here because monitoring the microphone twice would be pointless). If liveVolume is undefined, then we consider the volume to be 0.
