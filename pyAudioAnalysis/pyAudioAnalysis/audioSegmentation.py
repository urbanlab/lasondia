...
def plotSegmentationResults(flagsInd, flagsIndGT, classNames, mtStep, ONLY_EVALUATE=False):
    ...
    if not ONLY_EVALUATE:
        ...
        ax2.bar(numpy.array(range(len(classNames))) + 1, numpy.reshape(Percentages, -1))
        ...
        ax3.bar(numpy.array(range(len(classNames))) + 1, numpy.reshape(AvDurations, -1))
        fig.tight_layout()
        plt.show()
    return accuracy
...
def mtFileClassification(inputFile, modelName, modelType, plotResults=False, gtFile="", return_for_user=False):
    '''
